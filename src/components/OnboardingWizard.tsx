    ...
    <div className="fixed inset-0 bg-slate-900 bg-opacity-95 backdrop-blur-sm flex flex-col items-center justify-center p-4 z-[100]">
      {isEditMode && onBack && currentStep === 1 && (
        <button
          type="button"
          onClick={onBack}
          className="absolute top-4 left-4 sm:top-6 sm:left-6 z-[110] p-2 rounded-full hover:bg-slate-700/70 text-slate-300 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400"
      ...
      <div className="bg-slate-800 p-6 sm:p-10 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-purple-600 scrollbar-track-slate-700 min-h-0">
          {renderStepContent()}
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-700 flex justify-between items-center">
          <div className="flex items-center">
            {isEditMode && onBack && currentStep > 1 && (
                <button
                    type="button"
                    onClick={onBack}
                    className="px-6 py-2.5 bg-slate-500 hover:bg-slate-400 text-white font-medium rounded-lg shadow-md transition-colors duration-150 mr-3"
            ...
            {currentStep > 1 && (
              <button
                type="button"
                onClick={prevStep}
                className="px-6 py-2.5 bg-slate-600 hover:bg-slate-500 text-white font-medium rounded-lg shadow-md transition-colors duration-150"
          ...
          {currentStep < TOTAL_STEPS ? (
            ...
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-150"
