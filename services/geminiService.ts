...
CRITICAL: These suggestions MUST ABSOLUTELY be different from the movie "${promptQueryTitle}" (year ${promptQueryYear}) itself. Do NOT suggest "${promptQueryTitle}" (year ${promptQueryYear}) again in this list. The user has already seen it, and it should not be part of the new suggestions.
...
- Ensure all string values are correctly escaped.
- Provide exactly 3 movie recommendations. The 'matchScore' MUST reflect similarity to "${promptQueryTitle}".
- If you cannot find 3 suitable similar movies, provide fewer, but ensure the output is still a valid JSON array. If no additional similar movies can be found, return an empty array [].
`;
}
